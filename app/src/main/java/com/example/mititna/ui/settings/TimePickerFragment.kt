...
class TimePickerFragment : DialogFragment(), TimePickerDialog.OnTimeSetListener {
    ...
    private fun setupInitialTime(c: Calendar) {
        try {
            val sharedPreferences =
                activity?.let { PreferenceManager.getDefaultSharedPreferences(it /* Activity context */) }
    ...
    override fun onTimeSet(p0: TimePicker?, hourOfDay: Int, minute: Int) {
        val sharedPreferences =
            activity?.let { PreferenceManager.getDefaultSharedPreferences(it /* Activity context */) }
        sharedPreferences?.edit()?.putString(SettingsFragment.NOTIFICATION_TIME_PREFERENCE_KEY, "$hourOfDay:$minute")
            ?.apply()
    }
}